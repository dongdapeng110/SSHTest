    ...
    while ( SUCCEEDED( hr = pTables->Next( 1, &pTable, &celt ) ) && celt == 1 ) {
        ...
        if ( SUCCEEDED( pTable->QueryInterface( _uuidof( IDiaEnumSymbols ), (void**)&pSymbols ) ) ) {
            ...
            while ( SUCCEEDED( hr = pSymbols->Next( 1, &pSymbol, &celt ) ) && celt == 1 ) {
                ...
                CDiaBSTR name;
                if ( pSymbol->get_name( &name ) == S_OK && name != NULL ) {
                    printf( "sym name: %ws : %ws ", name, szTags[symTag] );
					if(symTag == SymTagData)
					{
						DWORD loc;
						if ( pSymbol->get_locationType( &loc ) != S_OK )
							Fatal( "Internal error, no locationType" );
						printf("locationType %d ", loc);
					}
					if(symTag == SymTagTypedef)
					{
						CDiaBSTR nameTypeDef;
						if(pSymbol->get_name(&nameTypeDef) == S_OK && nameTypeDef != NULL)
						{
							printf("\ntype def name: %ws ", nameTypeDef);
						}
						CComPtr<IDiaSymbol> pSymbolBase;
						pSymbol->get_type(&pSymbolBase);
						DWORD symTagBase;
						pSymbolBase->get_symTag( &symTagBase );

						int i = 0;
					}
					if(symTag == SymTagUDT)
					{
						CComPtr<IDiaSymbol> pSymbolLexParent;
						pSymbol->get_lexicalParent(&pSymbolLexParent);
						DWORD dwTag = 0;
						pSymbolLexParent->get_symTag(&dwTag);
						if(dwTag != SymTagExe)
						{
							int i = 0;
						}
						BOOL bNested = FALSE;
						pSymbol->get_nested(&bNested);
						if(bNested)
						{
							int i = 0;
						}
						...
						int i = 0;
						CComPtr<IDiaSymbol> pSubSymbol;
						CComPtr<IDiaEnumSymbols> pSubSymbols;
						HRESULT hrSub;
						ULONG celtSub = 0;
						pSymbol->findChildren(SymTagData, NULL, nsfCaseSensitive, &pSubSymbols);
						while(SUCCEEDED(hrSub = pSubSymbols->Next(1, &pSubSymbol, &celtSub)) && celtSub == 1)
						{
							DWORD symIndexSub;
							pSubSymbol->get_symIndexId( &symIndexSub );
							DWORD symTagSub;
							pSubSymbol->get_symTag( &symTagSub );
							CDiaBSTR nameSub;
							if(pSubSymbol->get_name(&nameSub) == S_OK && nameSub != NULL)
							{
								printf("\nsub sym name: %ws : %ws ", nameSub, szTags[symTagSub]);
							}
							pSubSymbol = NULL;
						}
					}
